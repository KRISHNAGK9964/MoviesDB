...
export default class List extends Component {
  ...
  handleFavourites = (movieObj) => {
    ...
    if(this.state.favMovies.includes(movieObj.id)){ // contains then delete
      localStorageMovies = localStorageMovies.filter((movie) => movie.id != movieObj.id);
    }
    else{ // not contains then add;
              ...
              {movie.map((movieObj) => (
                ...
                >
                  ...
                  <div className="button-wrapper">
                    {this.state.hover == movieObj.id && this.state.favMovies.includes(movieObj.id) ? (<a  className="btn btn-danger movie-button" onClick={()=>this.handleFavourites(movieObj)}>
                      Remove from Favourites
                    </a>) :this.state.hover == movieObj.id && (
            ...
            <nav aria-label="Page navigation example">
              <ul className="pagination">
                ...
                {this.state.parr.map((pageNum,index) => (
                    this.state.currPage == pageNum ? (<li className="page-item active" key={index}>
                      <a
                        className="page-link"
